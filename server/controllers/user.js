  ...
  User.findByIdAndUpdate(id, { active }, (err, userStored) => {
    if (err) {
      ...
      if (!userStored) {
        ...
        if (active === true) {
          res.status(200).send({ message: "Usuario activado exitosamente." });
        } else {
          res
            .status(200)
            .send({ message: "Usuario desactivado exitosamente." });
        }
      }
    }
  });
  ...
  User.findByIdAndRemove(id, (err, userDeleted) => {
    if (err) {
      ...
      if (!userDeleted) {
        ...
        res
          .status(200)
          .send({ message: "El usuario ha sido eliminado exitosamente." });
      }
    }
  });
}
